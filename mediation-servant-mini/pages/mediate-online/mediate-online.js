// pages/mediate-online/mediate-online.js
//获取应用实例
const app = getApp()
...
Page({
  ...
  getInfoCount: function () {
    const _this = this;
    wx.request({
        url: app.globalData.$ctx + '/wechatMini/mbmMediationReserveCount',
      data: {
  ...
  getInfoList: function () {
    const _this = this;
    wx.request({
        url: app.globalData.$ctx + '/wechatMini/mbmMediationReserveSearch',
      data: {
      ...
      success: function (res) {
        ...
        if (infoList != null) {
          for (let i = 0; i < infoList.length; i++) {
            var info = infoList[i];
            var html = {
              url: '../mediate-online/mediate-online-detail?id=' + info.id,
              date: info.date,
              name: info.name,
              mobile: info.mobile,
