...
Page({
  ...
  data: {
    ...
    setpArr: [
      ...
      { name: '办理方式', url: 'setp-five', id: 'five', dq: true, active: false, pass: false },
    ...
    typeArr: [
      { name: '线上办理', icon: 'icon-computer', active: true, isOnline: true },
      { name: '线下办理', icon: 'icon-down-line', active: false, isOnline: false }
    ]
  },
  choiceType: function (e) {
    ...
    wx.setStorageSync('isOnLine', arr[index].isOnline);
    console.log(wx.getStorageSync('isOnLine'));
  },
  getNext() {
    if (wx.getStorageSync('isOnLine')) {
  ...
  onShow: function () {
    ...
    if (wx.getStorageSync('isOnLine') === true) {
      let arr = this.data.typeArr;
      arr[0].active = true;
      arr[1].active = false;
      this.setData({
    ...
    } else if (wx.getStorageSync('isOnLine') === false) {
      let arr = this.data.typeArr;
      arr[0].active = false;
      arr[1].active = true;
      this.setData({
